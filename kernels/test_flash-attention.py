...
Source: https://github.com/ROCm/triton/blob/b36e072fc52e4a9d2222460519f4a8ff669d0f7e/python/perf-kernels/flash-attention.py
...
def test_benchmark():
    ...
    configs.append(
        triton.testing.Benchmark(x_names=x_names, x_vals=x_vals_list, line_arg='provider', line_vals=['triton'],
                                 ...
                                 plot_name=f'fused-attention-{mode}-d{head_size}-layoutbhsd',
    ...
    def bench_flash_attention(BATCH, HQ, HK, N_CTX_Q, N_CTX_K, D_HEAD, dtype, causal, mode, provider, device="cuda"):
        ...
        if varlen:
            ...
            q, k, v, input_metadata = input_helper(BATCH, HQ, HK, N_CTX_Q, N_CTX_K, D_HEAD, dtype, "bhsd")
    ...
    bench_flash_attention.run(save_path="./perf-artifacts/flash_attention", print_data=True)
